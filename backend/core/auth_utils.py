...
def authenticate_user(db: Session, username: str, password: str):
    ...
    2. If the user exists, it verifies the provided password against the stored hash.
    3. Returns the user object on success, otherwise returns None.
    """
    # Use your CRUD function to get the user
...
def get_current_admin_user(current_user: db_models.User = Depends(get_current_user)):
    """
    ...
    if current_user.role != db_models.UserRole.ADMIN:
        raise HTTPException(
            ...
            detail="The user does not have the necessary permissions for this resource.",
        )
    return current_user